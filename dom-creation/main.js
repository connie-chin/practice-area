...
const $row = document.querySelector('.row'); //only use $ for when query dom
if (!$row) throw new Error('The $row query failed'); //this if statement needs to be used anytime we query the dom, for typescript purposes. it doesnt know if this element exists or not.
for (let i = 0; i < pokedex.length; i++) {
  $row.append(renderPokemon(pokedex[i])); //here we are calling the function, passing in a single pokemon, and appending that to row
}
